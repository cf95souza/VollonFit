...
- **Correção de Políticas de Segurança (RLS - Academias e Gestores)**: Resolução do erro `new row violates row-level security policy for table "gym_academies"` e `"gym_academy_admins"`. O Master Admin logado (como `cf95.souza@gmail.com`) atua no Supabase sob a role `authenticated` (e não `anon`), o que violava as políticas públicas antigas. Implementamos as políticas exclusivas de controle total (`ALL`) para o email do Master Admin nas tabelas `gym_academies` e `gym_academy_admins`, liberando a criação e edição sem barreiras.
...
- **Portal B2B - Painel de Controle do Gestor da Academia (`AcademyDashboard.jsx`)**: Desenvolvimento completo e de altíssimo nível do painel corporativo integrado para franquias e academias contratantes do plano Enterprise:
  - *Visão Geral (Overview)*: Dashboard com cards de KPIs mostrando em tempo real o uso da cota de professores contratada, o total geral de alunos ativos integrados sob a tutela da academia, e os detalhes de faturamento da assinatura mensal B2B. Acompanha uma barra de progresso interativa e relatórios de uso.
  - *Gestão do Corpo Docente*: Painel para listar e gerenciar todos os professores associados à academia. Possibilita o cadastro de novos profissionais com validação de limite de licenças da cota (bloqueando o cadastro e instruindo sobre o upgrade de cota caso o limite seja excedido), edição de dados (Nome, Email, Telefone, Cota individual de alunos) e bloqueio/desbloqueio in-place de acesso ao sistema.
  - *Relatório Geral de Alunos*: Listagem unificada e pesquisável de todos os alunos cadastrados por todos os professores pertencentes à academia corporativa, permitindo auditorias e visualizações rápidas de status.
  - *Identidade Visual Corporativa*: Aba de configurações que permite ao gestor atualizar o nome de exibição, CNPJ de faturamento e link do logotipo oficial da marca, personalizando o cabeçalho e layout do portal.
- **Link de Auto-Cadastro de Alunos para Professor Premium (Fase 35)**:
  - *Link de Convite Exclusivo*: A aba de Configurações do Professor no plano Premium exibe uma nova seção com seu link exclusivo de auto-cadastro (ex: `/login?invite=teacher_id`) com cópia em um clique e feedback de toast. Para professores no plano Basic, a funcionalidade é exibida com um painel bloqueado estimulando o upgrade para o plano Premium.
  - *Formulário de Auto-Cadastro*: Ao abrir o link de convite, o aluno é direcionado para a tela de login que se adapta visualmente, apresentando um formulário de cadastro amigável onde ele digita Nome, Nome de Usuário (Login) e Senha.
  - *Associação Relacional & Validação de Cota*: O aluno criado é automaticamente associado ao ID do professor convidante no banco de dados. O fluxo também valida o limite de alunos (`quota_limit`) do professor em tempo real, travando novas matrículas autônomas e instruindo o aluno a contatar o profissional caso o limite seja excedido.
